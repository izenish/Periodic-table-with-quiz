...
while(!kbhit())
 {
	...
	{cleardevice();
		printf("\nAtoms are the basic units of matter and the defining structure of element. It is composed of protons, neutrons and electrons.");		r();
...
int r()
{	
	  outtextxy(25,35,"Back");
	while(!kbhit())
 {
	coo(&b,&x,&y);
	if(b==1)
	{   
		if(x>=15 && y>=25 && x<=45 && y<=45)  
